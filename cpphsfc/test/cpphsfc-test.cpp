...
{
    // Write out the gdl to a temporary file so that we can test
    // that loading from a file works correctly.
    namespace bfs=boost::filesystem;
    bfs::path gdlpath = bfs::unique_path();
    try {
        bfs::ofstream tmpo(gdlpath);
        BOOST_CHECK(!tmpo.fail());
        tmpo << g_tictactoe;
        tmpo.close();
    ...
    {
        if (bfs::is_regular_file(gdlpath)) bfs::remove(gdlpath);
        throw;
    }
...
{
    Game game_a(g_tictactoe);
    Game game_b(g_tictactoe);
