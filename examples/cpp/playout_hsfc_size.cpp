...
void run(const std::string& gdlfilename, unsigned int round_length)
{
    ...
    std::vector<HSFC::JointMove> joint_moves;
...
/*
        // Make a 1000 copies of base_state
        std::vector<HSFC::State> states(100000, base_state);
        unsigned int counter = 0;
        for (std::vector<HSFC::State>::iterator i = states.begin(); i != states.end(); ++i)
        {
            goals = i->playout();
            ++counter;
        }
        std::cerr << "Counted: " << counter << std::endl;
*/
...
//            base_state.legals();
        ...
        for (unsigned int i = 0; i < 10000; ++i)
            joint_moves.insert(joint_moves.end(), jms.begin(), jms.end());
        base_state.play(jms[0]);
        ++numjm;
    }
    std::cerr << "Number of joint moves: " << joint_moves.size() << std::endl;
